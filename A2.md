...
---
- Authentication ways : 
	1. Traditional Ways (Without using passport-js or similar library) 
	    - Using Cookies based auth only (Client side) [npm package : cookie-parser]
	    - Using Session based auth (Server side & store SID using cookie, hence also called session-cookie) [npm package : cookie-parser, express-session]
	    - Using Session based auth (For storage request header not cookie) [npm package :  express-session]
	2. Modern Way (Using passport-js library which consist of 3 part strategy, session & middleware)
	   NOTE : Install (npm) -> Configure (Strategy Logic+Sessions) -> Register/Use/Middleware (.initialize,.use,.authenticate)
	   - Using Cookies or Session based username/password auth [npm : passport-local strategy] (Default is session based & requires serialize/deserialize)
	   - Using Token based auth [npm : passport-jwt, jsonwebtoken, bcryptjs]
	   		- Token storage (cookies) - stateful jwt
	   		- Token storage (Session-cookie/server side) - stateful jwt
	   		- Token storage (local storage/session storage) - stateless jwt [default]
	   		- Token storage (Req. Headers) - stateless jwt [default] //Recommended
	   - For Delegated/OAuth 2.0 (Google,facebook, twitter, github etc.) - their respective passport strategies
	   - For Federated/OpenID Auth (Google etc.) - their respective passport strategies npm packages
---
- Authentication Flow (Login & Registration Form Managament System)
  - NOTE 1 : Each route could have multiple possiblities based on HTTP method. Eg. "/signup" route have GET, POST, PUT, etc. Hence when not using any lib/fw multiple api calls are used eg. for signup.html (GET "/signup"), for actual registration (POST "/signup"). If we use library/fw like react this extra call gets reduced as view/URL-Navigation/Routing of templates get handled by FE lib. and is recommended as best practice.

  - NOTE 2 : API Connects BE/FE like this : Client HTTP Method (Fetch/axios/href/etc) <--> API Calls/Routes/Endpoints/URL <---> BE Logic/Route Handler
  - Client (HTTP Request Method/cURL/Postman/firefox-browser/JS Code) [Other way to hit API : Webhook, Websocket]
	  - Without using Lib/FW : HTML-CSS-JS OR using template engine for view like pug,hbs,ejs etc.
		  - View (FE : Login Page/Registration Form - GET Requests)
			  - HTML <a href="signup.html"> : URL Navigation of page w/o sending req. to server [GET] //Not Recommended
			  - JS : window.location.href vs. history.pushState() //Not Recommended
			  - Server Res. (expessJS) : res.redirect/render/sendFile [GET - app.get()] OR express.static() //Recommended
		  - API Calls (FE : Login & Signup/Register)
			  - <form action="login.html" method=POST> : Submitting data in req.body using submit button [POST, GET]
			  - HTTP Req. Lib. : XMLHttpRequest, fetch, axios, superagent etc. [All Methods : GET, POST, PUT etc.] //Recommended
	  - Using Lib/FW : React, Angular, Vue etc. // Recommended as BE should only deal with model,controller,service,routes
		  - View (FE : Login Page/Registration Form) : Respective Routing packages for URL Navigation
			  - Eg. React have BrowseRouter in which respective view gets rendered for url ie. "/register" -> register.jsx
		  - API Calls (FE : Login & Signup/Submit Button) - Using XMLHttpRequest, fetch, axios, superagent etc. HTTP req. lib
  - Server (Response/Backend/Expressjs) :
	  - res.json : only json data gets sent back // Recommended
	  - res.redirect : For URL Navigation to other Route/API Endpoint
	  - res.render : To render template data (used for template engine like pug,hbs,ejs etc)
	  - res.send : for sending only text string in response
	  - res.sendFile : for sending File data as response (html, txt etc.)
	  - express.static : to serve static index.html file from views folder eg. express.static(path.join(__dirname,'View')));
